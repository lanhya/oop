var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var Operation = /** @class */ (function () {
    function Operation(number1, number2) {
        this.number1 = number1;
        this.number2 = number2;
    }
    return Operation;
}());
var AddOperation = /** @class */ (function (_super) {
    __extends(AddOperation, _super);
    function AddOperation(number1, number2) {
        return _super.call(this, number1, number2) || this;
    }
    AddOperation.prototype.doOperation = function () {
        return this.number1 + this.number2;
    };
    return AddOperation;
}(Operation));
var MultiplayOperation = /** @class */ (function (_super) {
    __extends(MultiplayOperation, _super);
    function MultiplayOperation(number1, number2) {
        return _super.call(this, number1, number2) || this;
    }
    MultiplayOperation.prototype.doOperation = function () {
        return this.number1 * this.number2;
    };
    return MultiplayOperation;
}(Operation));
var sum = new AddOperation(3, 3);
console.log(sum.doOperation());
var mulTi = new MultiplayOperation(6, 6);
console.log(mulTi.doOperation());
